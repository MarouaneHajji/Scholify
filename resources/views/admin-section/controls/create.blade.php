...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Create Control</h1>
            <p class="mt-2 text-sm text-gray-600">Add a new control to {{ $module->name }}</p>
        </div>
        <a href="{{ route('admin.branches.modules.show', [$module->branch, $module]) }}" 
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
            <form action="{{ route('admin.modules.controls.store', $module) }}" method="POST" class="p-8">
                @csrf

                <div class="space-y-6">
                    <!-- Control Name -->
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-900">Control Name</label>
                        <input type="text" 
                               name="name" 
                               id="name" 
                               value="{{ old('name') }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Max Grade -->
                        <div>
                            <label for="max_grade" class="block text-sm font-medium text-gray-900">Maximum Grade</label>
                            <div class="mt-2 relative rounded-lg">
                                <input type="number" 
                                       name="max_grade" 
                                       id="max_grade" 
